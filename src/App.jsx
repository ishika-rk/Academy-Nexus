            ...
            {rows.length === 0 ? (
              ...
              <tr key={row.id ?? i} style={{ background: i % 2 === 0 ? "#fff" : C.surfaceAlt }}>
                {columns.map(col => {
                  ...
                  // TEMPORARY debug aid, 2026-08-25: descriptors aren't showing up for some
                  // synced interviews — when a rubric cell has no descriptor, fall back to
...
function fillRubricColumns(row, iv, columns) {
  ...
  for (const col of columns) {
    ...
    if (/Avg$/.test(col.key)) {
      ...
      // TEMPORARY debug aid, 2026-08-25: descriptors aren't showing up for some synced
      // interviews — stash what fieldKey we looked up and the raw domain object so the cell
