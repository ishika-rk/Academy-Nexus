            ...
            {rows.length === 0 ? (
              ...
              <tr key={row.id ?? i} style={{ background: i % 2 === 0 ? "#fff" : C.surfaceAlt }}>
                {columns.map(col => {
                  ...
                  // TEMPORARY debug aid, 2026-08-25 (round 2): re-checking after the Interview
                  // App team's fix — when a rubric cell has no descriptor, fall back to dumping
                  // the raw domain object we looked it up in. Remove once diagnosed.
...
function fillRubricColumns(row, iv, columns) {
  ...
  for (const col of columns) {
    ...
    if (/Avg$/.test(col.key)) {
      ...
      // TEMPORARY debug aid, 2026-08-25 (round 2): re-checking after the Interview App team's
      // fix — stash what fieldKey we looked up and the raw domain object so the cell click
      // popup can show it when no descriptor is found. Remove once diagnosed.
